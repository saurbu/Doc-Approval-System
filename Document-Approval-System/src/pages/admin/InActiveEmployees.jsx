...
import { MoreVertical, Phone, Building2, Mail, ChevronLeft, ChevronRight, UserX, Edit2, Power } from 'lucide-react'
  ...
  if (loading) {
    return (
      <div className="py-20 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-slate-500 text-sm font-medium">Loading inactive accounts...</p>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filterEmployee.length > 0 ? (
          filterEmployee.map((emp) => (
            <div 
              key={emp._id} 
              className="bg-white rounded-2xl border border-slate-100 p-5 shadow-xs hover:shadow-md hover:-translate-y-0.5 transition duration-200 flex flex-col justify-between relative opacity-90 hover:opacity-100"
            >
              <div>
                {/* Header ID & Status */}
                      ...
                      {openMenu === emp._id && (
                        <>
                          ...
                          <div className="absolute right-0 top-7 w-44 bg-white rounded-xl shadow-xl border border-slate-100 p-1.5 z-50 animate-fadeIn">
                            ...
                            <button
                              className="w-full flex items-center gap-2 px-3 py-2 text-xs font-semibold text-emerald-600 hover:bg-emerald-50 rounded-lg transition cursor-pointer"
                ...
                <div className="mt-4 mb-4">
                  <h3 className="font-bold text-lg text-slate-800 line-clamp-1">{emp.name}</h3>
          ...
          <div className="col-span-full py-16 text-center">
            ...
            <h4 className="text-base font-semibold text-slate-700">No inactive employees found</h4>
            <p className="text-xs text-slate-400 mt-1">There are no inactive staff members matching your criteria.</p>
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-rose-600 text-white hover:bg-rose-700 text-xs font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer"
            >
              <span>Next</span>
